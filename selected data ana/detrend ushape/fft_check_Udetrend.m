...
for ti = 1:num_permute
    % permute for detrended_data
    permute_detrend_data = detrended_data(:,randperm(size(detrended_data,2)));
    %permute_detrend_data = data(:,randperm(size(data,2)));
    var_soa = 10; % in ms
...
% plot detrend data
figure;
s3 = shadedErrorBar(soa,mean(detrended_data,1)/100,(std(detrended_data,0,1)/100) ./ sqrt(size(detrended_data,1)),'lineProps',{'-','color',color(2,:)});
...
text_x = 29; % right
text_y =max(mean_power)+0.001 ; % legend position
...
hold on
% plot(max_freqs, sorted_vals+0.002, '*', 'Color', color(3,:), 'MarkerSize', 5, 'LineWidth', 0.8);
